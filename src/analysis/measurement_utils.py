...
def calculate_snr(
    ...
    """
    Calculate Signal-to-Noise Ratio for a detrended spectrum.

    Approach:
    - Signal: The low-frequency oscillations (fringes).
    - Noise: High-frequency residual after subtracting smoothed fringes.

    Args:
        wavelengths: Wavelength array (nm).
        detrended_signal: High-pass filtered reflectance.
        noise_estimation_width_nm: Smoothing width used to isolate fringes.
    ...
    # Isolate stable fringes by smoothing
    ...
    sig_var = np.var(clean_signal)
    noise_var = np.var(noise)

    if noise_var <= 0:
        return 1000.0  # High SNR for noiseless signal
