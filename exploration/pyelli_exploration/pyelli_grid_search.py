...
class PyElliResult:
    ...
    peak_count_delta: int = (
        0  # Absolute difference between measurement and theoretical peak counts
    )
    mean_delta_nm: float = 0.0  # Mean delta between matched peaks (alignment quality)
    oscillation_ratio: float = (
        1.0  # Ratio of theoretical to measured oscillation amplitude
    )
    ...
    snr: float = 0.0  # Signal-to-Noise Ratio (Power ratio)
...
def transfer_matrix_reflectance_fallback(
    ...
    for i, wl in enumerate(wavelengths_nm):
        ...
        for j in range(1, len(N) - 1):
            ...
            L_j = np.array(
                [[np.exp(-1j * delta), 0], [0, np.exp(1j * delta)]], dtype=complex
            )
        ...
        I_final = np.array([[1, r_final], [r_final, 1]], dtype=complex) / t_final
...
def calculate_peak_based_score(
    ...
    """
    from src.analysis.measurement_utils import calculate_snr
    ...
    if correlation < min_correlation:
        ...
        if correlation < 0:
            ...
            ) * 0.3  # Max 0.3 for below-threshold
    else:
        # Above threshold: score scales from 0.7 to 1.0
        correlation_score = 0.7 + 0.3 * (
            (correlation - min_correlation) / (1.0 - min_correlation)
        )
    ...
    try:
        meas_detrended = detrend_signal(
            wavelengths, measured, cutoff_frequency, filter_order
        )
        theo_detrended = detrend_signal(
            wavelengths, theoretical, cutoff_frequency, filter_order
        )
    except Exception:
        ...
        return {
            ...
            "matched_peaks": 0,
            "mean_delta_nm": 0.0,
            "snr": calculate_snr(wavelengths, meas_detrended)
            if "meas_detrended" in locals()
            else 0.0,
        }

    # Detect peaks
    meas_peaks_df = detect_peaks(
        wavelengths, meas_detrended, prominence=peak_prominence
    )
    theo_peaks_df = detect_peaks(
        wavelengths, theo_detrended, prominence=peak_prominence
    )

    meas_peaks = (
        meas_peaks_df["wavelength"].to_numpy(dtype=float)
        if len(meas_peaks_df) > 0
        else np.array([], dtype=float)
    )
    theo_peaks = (
        theo_peaks_df["wavelength"].to_numpy(dtype=float)
    ...
    if len(matched_meas) >= 3:
        ...
        matched_meas_amplitudes = (
            meas_peaks_df["amplitude"].iloc[matched_meas].to_numpy(dtype=float)
        )
        matched_theo_amplitudes = (
            theo_peaks_df["amplitude"].iloc[matched_theo].to_numpy(dtype=float)
        )
    ...
    if meas_count == 0:
        ...
        count_ratio = (
            min(meas_count, theo_count) / max(meas_count, theo_count)
            if max(meas_count, theo_count) > 0
            else 0.0
        )
    ...
    if deltas.size == 0:
        ...
        if unmatched_measurement == 0 and unmatched_theoretical == 0:
            ...
            if total_peaks > 0:
                ...
                peak_delta_score = 0.05 * (
                    1.0 - unmatched_ratio
                )  # Small score for similar counts
            else:
                peak_delta_score = 0.0
    else:
    ...
    snr_val = calculate_snr(wavelengths, meas_detrended)
    ...
    very_early_rmse = (
        float(np.sqrt(np.mean(residual[very_early_mask] ** 2)))
        if very_early_mask.any()
        else rmse
    )
    early_rmse = (
        float(np.sqrt(np.mean(residual[early_mask] ** 2))) if early_mask.any() else rmse
    )
    mid_rmse = (
        float(np.sqrt(np.mean(residual[mid_mask] ** 2))) if mid_mask.any() else rmse
    )
    late_rmse = (
        float(np.sqrt(np.mean(residual[late_mask] ** 2))) if late_mask.any() else rmse
    )
    ...
    weighted_rmse = (
        0.30 * very_early_rmse + 0.20 * early_rmse + 0.25 * mid_rmse + 0.25 * late_rmse
    )
...
def calculate_drift_metrics(
    ...
    if num_matched < min_peaks_for_drift:
        logger.debug(
            f"📊 Drift analysis skipped: only {num_matched} matched peaks (need {min_peaks_for_drift})"
        )
    ...
    if abs(denominator) < 1e-10:
        ...
        peak_drift_r_squared = (
            1.0 - (ss_residual / ss_total) if ss_total > 1e-10 else 0.0
        )
        peak_drift_r_squared = max(0.0, peak_drift_r_squared)

    # Flag if slope is significant AND systematic (high R²)
    peak_drift_flagged = (
        abs(peak_drift_slope) > peak_drift_slope_threshold
        and peak_drift_r_squared > r_squared_threshold
    )
    ...
    safe_meas_amps = np.where(
        np.abs(matched_meas_amplitudes) > 1e-10, matched_meas_amplitudes, 1e-10
    )
    ...
    if abs(denominator) < 1e-10:
        ...
        amplitude_drift_r_squared = (
            1.0 - (ss_residual_amp / ss_total_amp) if ss_total_amp > 1e-10 else 0.0
        )
    ...
    amplitude_drift_flagged = (
        abs(amplitude_drift_slope) > amplitude_drift_slope_threshold
        and amplitude_drift_r_squared > r_squared_threshold
    )
...
def calculate_monotonic_alignment_score(
    ...
    focus_mask = (wavelengths >= focus_wavelength_min) & (
    ...
    score = 0.40 * shape_score + 0.40 * closeness_score + 0.20 * crossing_score
...
def _get_candidate_sort_key(
    result: PyElliResult, min_matched_peaks: int = 0, meas_peaks_count: int = 0
) -> tuple:
    """
...
def _get_measured_peaks_count(wavelengths: np.ndarray, measured: np.ndarray) -> int:
    ...
    try:
        ...
        meas_detrended = detrend_signal(
            wavelengths, measured, cutoff_frequency=0.008, filter_order=3
        )
...
def _worker_initializer(
    ...
    warnings.filterwarnings(
        "ignore", message=".*ScriptRunContext.*", category=UserWarning
    )
    warnings.filterwarnings(
    ...
    class ScriptRunContextFilter(worker_logging.Filter):
        def filter(self, record):
            ...
            return (
                "ScriptRunContext" not in msg and "missing ScriptRunContext" not in msg
            )

    for handler in root_logger.handlers:
        handler.addFilter(ScriptRunContextFilter())
    worker_logging.getLogger(
        "streamlit.runtime.scriptrunner_utils.script_run_context"
    ).setLevel(worker_logging.CRITICAL)
    worker_logging.getLogger("streamlit.runtime.scriptrunner_utils").setLevel(
        worker_logging.CRITICAL
    )
    ...
    lipid_n = np.interp(
        wavelengths,
        material_data["lipid"]["wavelength_nm"],
        material_data["lipid"]["n"],
    )
    lipid_k = np.interp(
        wavelengths,
        material_data["lipid"]["wavelength_nm"],
        material_data["lipid"]["k"],
    )
    water_n = np.interp(
        wavelengths,
        material_data["water"]["wavelength_nm"],
        material_data["water"]["n"],
    )
    water_k = np.interp(
        wavelengths,
        material_data["water"]["wavelength_nm"],
        material_data["water"]["k"],
    )
    mucus_n = np.interp(
        wavelengths,
        material_data["mucus"]["wavelength_nm"],
        material_data["mucus"]["n"],
    )
    mucus_k = np.interp(
        wavelengths,
        material_data["mucus"]["wavelength_nm"],
        material_data["mucus"]["k"],
    )
    substratum_n = np.interp(
        wavelengths,
        material_data["substratum"]["wavelength_nm"],
        material_data["substratum"]["n"],
    )
    substratum_k = np.interp(
        wavelengths,
        material_data["substratum"]["wavelength_nm"],
        material_data["substratum"]["k"],
    )
...
def _evaluate_combination_fast(
    ...
    try:
        ...
        theo_focus_scaled = (
            theoretical_scaled[focus_mask]
            if focus_mask.sum() > 0
            else theoretical_scaled
        )

        if focus_mask.sum() > 0:
            if np.std(meas_focus_scaled) > 1e-10 and np.std(theo_focus_scaled) > 1e-10:
                quick_corr = float(
                    np.corrcoef(meas_focus_scaled, theo_focus_scaled)[0, 1]
                )
        ...
        if quick_corr < 0.7:
            ...
            try:
                meas_detrended_quick = detrend_signal(
                    wl_focus, meas_focus_scaled, 0.008, 3
                )
                theo_detrended_quick = detrend_signal(
                    wl_focus, theo_focus_scaled, 0.008, 3
                )
            ...
            simple_score = (
                0.35 * rmse_score + 0.35 * amplitude_score + 0.30 * correlation_score
            )
        ...
        return PyElliResult(
            ...
            amplitude_drift_r_squared=float(
                score_result.get("amplitude_drift_r_squared", 0.0)
            ),
            amplitude_drift_flagged=bool(
                score_result.get("amplitude_drift_flagged", False)
            ),
            snr=float(score_result.get("snr", 0.0)),
        )
    except Exception as e:
        return None
...
def _evaluate_single_combination(
    ...
    try:
        ...
        if quick_corr < 0.7:
            ...
            residual = meas_focus_scaled - theo_focus_scaled
            rmse = float(np.sqrt(np.mean(residual**2)))
            correlation = score_result.get(
                "correlation", quick_corr
            )  # Use from score_result if available
        ...
        logger.debug(
            f"Error evaluating combination (L={lipid}, A={aqueous}, R={roughness_angstrom}Å): {e}"
        )
        return None
...
class PyElliGridSearch:
    ...
    def __init__(
        ...
        logger.info(
            f"✅ Loaded tear film materials: lipid={lipid_file}, water={water_file}, mucus={mucus_file}, substratum={substratum_file}"
        )
    ...
    def _get_nk(
        self, mat_df: pd.DataFrame, wavelengths: np.ndarray
    ...
    def run_grid_search(
        ...
        lipid_range: Tuple[float, float, float] = (
            ...
            250,
            10,
        ),  # Standard range: 9-250 nm
        aqueous_range: Tuple[float, float, float] = (
            800,
            12000,
            300,
        ),  # Standard range: 800-12000 nm
        roughness_range: Tuple[float, float, float] = (
            6000,
            7000,
            100,
        ),  # Optimized: good fits at ≥6000 Å
        ...
        max_combinations: Optional[
            int
        ] = 10000,  # Max combinations for Coarse/Dynamic search
    ) -> List[PyElliResult]:
        """
        ...
        logger.info(
            f"   - Lipid Range: {lipid_range[0]:.1f} - {lipid_range[1]:.1f} nm (step: {lipid_range[2]:.1f} nm)"
        )
        logger.info(
            f"   - Aqueous Range: {aqueous_range[0]:.1f} - {aqueous_range[1]:.1f} nm (step: {aqueous_range[2]:.1f} nm)"
        )
        logger.info(
            f"   - Roughness Range: {roughness_range[0]:.1f} - {roughness_range[1]:.1f} Å (step: {roughness_range[2]:.1f} Å)"
        )
        logger.info(
            f"   - Max Combinations: {max_combinations:,}"
            if max_combinations
            else "   - Max Combinations: None (unlimited)"
        )
    ...
    def _run_coarse_search(
        ...
        logger.info(
            f"   - Max Combinations Limit: {max_combinations:,}"
            if max_combinations
            else "   - Max Combinations Limit: None (unlimited)"
        )
        logger.info(f"   - Fine Search: {fine_search}")
        logger.info(f"   - Top K Results: {top_k}")
        logger.info("=" * 80)
        ...
        logger.info(
            f"   Input step sizes: Lipid={lipid_range[2]:.1f}nm, Aqueous={aqueous_range[2]:.1f}nm, Roughness={roughness_range[2]:.1f}Å"
        )

        lipid_values = np.arange(lipid_range[0], lipid_range[1] + 1, lipid_range[2])
        aqueous_values = np.arange(
            aqueous_range[0], aqueous_range[1] + 1, aqueous_range[2]
        )
        roughness_values_angstrom = np.arange(
            roughness_range[0], roughness_range[1] + 1, roughness_range[2]
        )

        logger.info(
            f"   Generated before filtering: Lipid={len(lipid_values)}, Aqueous={len(aqueous_values)}, Roughness={len(roughness_values_angstrom)}"
        )

        # Filter to user-specified ranges (already bounded by ADOM limits in UI)
        lipid_values = lipid_values[
            (lipid_values >= lipid_range[0]) & (lipid_values <= lipid_range[1])
        ]
        aqueous_values = aqueous_values[
            (aqueous_values >= aqueous_range[0]) & (aqueous_values <= aqueous_range[1])
        ]
        roughness_values_angstrom = roughness_values_angstrom[
            (roughness_values_angstrom >= roughness_range[0])
            & (roughness_values_angstrom <= roughness_range[1])
        ]

        logger.info(
            f"   After range filtering: Lipid={len(lipid_values)}, Aqueous={len(aqueous_values)}, Roughness={len(roughness_values_angstrom)}"
        )

        if (
            len(lipid_values) == 0
            or len(aqueous_values) == 0
            or len(roughness_values_angstrom) == 0
        ):
            logger.warning(
                f"⚠️ Parameter ranges resulted in empty grid! Lipid: {len(lipid_values)}, Aqueous: {len(aqueous_values)}, Roughness: {len(roughness_values_angstrom)}"
            )
            return []
        ...
        if total > max_combinations:
            logger.warning(
                f"⚠️ Coarse grid too large ({total:,} combinations). Limiting to {max_combinations:,} by sampling..."
            )
            logger.info(
                f"   Before sampling: {len(lipid_values)} lipid × {len(aqueous_values)} aqueous × {len(roughness_values_angstrom)} roughness = {total:,} combinations"
            )
            ...
            if len(lipid_values) > target_per_dim:
                indices = np.linspace(
                    0, len(lipid_values) - 1, target_per_dim, dtype=int
                )
                lipid_values = lipid_values[indices]
                logger.info(
                    f"   Lipid sampled: {lipid_before} → {len(lipid_values)} values"
                )
            if len(aqueous_values) > target_per_dim:
                indices = np.linspace(
                    0, len(aqueous_values) - 1, target_per_dim, dtype=int
                )
                aqueous_values = aqueous_values[indices]
                logger.info(
                    f"   Aqueous sampled: {aqueous_before} → {len(aqueous_values)} values"
                )
            if len(roughness_values_angstrom) > target_per_dim:
                indices = np.linspace(
                    0, len(roughness_values_angstrom) - 1, target_per_dim, dtype=int
                )
                roughness_values_angstrom = roughness_values_angstrom[indices]
                logger.info(
                    f"   Roughness sampled: {roughness_before} → {len(roughness_values_angstrom)} values"
                )

            total = (
                len(lipid_values) * len(aqueous_values) * len(roughness_values_angstrom)
            )
            logger.info(
                f"   After sampling: {len(lipid_values)} lipid × {len(aqueous_values)} aqueous × {len(roughness_values_angstrom)} roughness = {total:,} combinations"
            )
        ...
        logger.info("-" * 80)
        logger.info(
            f"🔧 Step Sizes: Lipid={lipid_range[2]:.1f}nm, Aqueous={aqueous_range[2]:.1f}nm, Roughness={roughness_range[2]:.1f}Å"
        )
        logger.info(f"📈 Grid Dimensions:")
        logger.info(
            f"   - Lipid values: {len(lipid_values)} ({lipid_values[0]:.1f} to {lipid_values[-1]:.1f} nm)"
        )
        logger.info(
            f"   - Aqueous values: {len(aqueous_values)} ({aqueous_values[0]:.1f} to {aqueous_values[-1]:.1f} nm)"
        )
        logger.info(
            f"   - Roughness values: {len(roughness_values_angstrom)} ({roughness_values_angstrom[0]:.1f} to {roughness_values_angstrom[-1]:.1f} Å)"
        )
        ...
        with ProcessPoolExecutor(
            ...
            for future in as_completed(futures):
                ...
                if result is not None:
                    ...
                    if (
                        result.correlation >= min_correlation_filter
                        and result.rmse <= 0.002
                    ):  # Reject fits with RMSE > 0.002
        ...
        if filtered_out > 0:
            logger.info(
                f"   Filtered out: {filtered_out:,} (correlation < {min_correlation_filter} or RMSE > 0.002)"
            )

        if len(results) == 0:
            logger.warning(
                "⚠️ No results passed filters! All candidates had correlation < 0.85 or RMSE > 0.002. Try expanding search ranges or relaxing filters."
            )
            return []
        ...
        if fine_search and good_candidates:
            ...
            logger.info(
                f"🎯 Refining top {num_refine} candidates (RMSE < 0.0015) with fine search..."
            )
            ...
            logger.info(
                f"🎯 Refining top {num_refine} candidates with fine search (no excellent candidates found)..."
            )
        ...
        if top_candidates:
            ...
            remaining_budget = (
                (max_combinations - coarse_used) if max_combinations else None
            )
            ...
            for rank, candidate in enumerate(top_candidates, 1):
                logger.info(
                    f"   Candidate #{rank}: Score={candidate.score:.4f}, Corr={candidate.correlation:.3f}, "
                ...
                fine_roughness_step = max(
                    5.0, roughness_range[2] * fine_refinement_factor
                )

                # Search in a wider window around the candidate (±3 coarse steps for better coverage)
                fine_lipid_min = max(
                    lipid_range[0], candidate.lipid_nm - 3 * lipid_range[2]
                )
                fine_lipid_max = min(
                    lipid_range[1], candidate.lipid_nm + 3 * lipid_range[2]
                )
                fine_lipid_values = np.arange(
                    fine_lipid_min, fine_lipid_max + fine_lipid_step, fine_lipid_step
                )

                fine_aqueous_min = max(
                    aqueous_range[0], candidate.aqueous_nm - 3 * aqueous_range[2]
                )
                fine_aqueous_max = min(
                    aqueous_range[1], candidate.aqueous_nm + 3 * aqueous_range[2]
                )
                fine_aqueous_values = np.arange(
                    fine_aqueous_min,
                    fine_aqueous_max + fine_aqueous_step,
                    fine_aqueous_step,
                )

                fine_roughness_min = max(
                    roughness_range[0], candidate.mucus_nm - 3 * roughness_range[2]
                )
                fine_roughness_max = min(
                    roughness_range[1], candidate.mucus_nm + 3 * roughness_range[2]
                )
                fine_roughness_values = np.arange(
                    fine_roughness_min,
                    fine_roughness_max + fine_roughness_step,
                    fine_roughness_step,
                )

                # Filter to user-specified ranges (already bounded by ADOM limits in UI)
                fine_lipid_values = fine_lipid_values[
                    (fine_lipid_values >= lipid_range[0])
                    & (fine_lipid_values <= lipid_range[1])
                ]
                fine_aqueous_values = fine_aqueous_values[
                    (fine_aqueous_values >= aqueous_range[0])
                    & (fine_aqueous_values <= aqueous_range[1])
                ]
                fine_roughness_values = fine_roughness_values[
                    (fine_roughness_values >= roughness_range[0])
                    & (fine_roughness_values <= roughness_range[1])
                ]
                ...
                for lipid in fine_lipid_values:
                    for aqueous in fine_aqueous_values:
                        for roughness_angstrom in fine_roughness_values:
                            ...
                            if (
                                abs(lipid - candidate.lipid_nm) < 0.1
                                and abs(aqueous - candidate.aqueous_nm) < 1.0
                                and abs(roughness_angstrom - candidate.mucus_nm) < 1.0
                            ):
                                continue
            ...
            if max_combinations and remaining_budget and total_fine > remaining_budget:
                logger.warning(
                    f"⚠️ Fine search combinations ({total_fine:,}) exceed remaining budget ({remaining_budget:,})"
                )
                logger.warning(
                    f"   Limiting to {remaining_budget:,} combinations by random sampling..."
                )
                random.seed(42)
                ...
                logger.warning(
                    f"⚠️ Fine search combinations ({total_fine:,}) exceed max limit ({max_combinations:,})"
                )
                logger.warning(
                    f"   Limiting to {max_combinations:,} combinations by random sampling..."
                )
                random.seed(42)
            ...
            if fine_combinations:
                num_workers = os.cpu_count() or 4
                logger.info(
                    f"🚀 Starting parallel evaluation of {total_fine:,} combinations with {num_workers} workers..."
                )
                ...
                with ProcessPoolExecutor(
                    ...
                    for future in as_completed(futures):
                        ...
                        if (
                            fine_completed % progress_interval == 0
                            or fine_completed == total_fine
                        ):
                            ...
                            eta = (
                                (total_fine - fine_completed) / rate if rate > 0 else 0
                            )
                            logger.info(
                                f"   Progress: {fine_completed}/{total_fine} ({progress_pct:.1f}%) | "
                                f"Elapsed: {elapsed:.1f}s | Rate: {rate:.1f} comb/s | ETA: {eta:.1f}s"
                            )
            ...
            all_results.sort(
                key=lambda r: _get_candidate_sort_key(r, 0, 0), reverse=True
            )
            ...
            logger.info("=" * 80)
            logger.info(
                f"⏱️  Total time: {total_elapsed:.1f}s ({total_elapsed / 60:.1f} minutes)"
            )
            ...
            if unique_results:
                logger.info(
                    f"🏆 Best result: Score={unique_results[0].score:.4f}, Corr={unique_results[0].correlation:.3f}, "
            ...
            min_matched_peaks = max(
                1, int(meas_peaks_count * 0.5)
            )  # Require at least 50% of measured peaks
            ...
            sort_key_func = lambda r: _get_candidate_sort_key(
                r, min_matched_peaks, meas_peaks_count
            )
            unique_results.sort(key=sort_key_func, reverse=True)

            # Filter candidates that meet minimum matched peaks requirement
            candidates_meeting_min = [
                r for r in unique_results if r.matched_peaks >= min_matched_peaks
            ]
            ...
            if len(candidates_meeting_min) >= 10:
                ...
                logger.info(
                    f"📊 Filtering: {len(candidates_meeting_min)} candidates meet minimum matched peaks requirement ({min_matched_peaks} of {meas_peaks_count} measured peaks)"
                )
            else:
                candidate_pool = unique_results
                logger.warning(
                    f"⚠️ Only {len(candidates_meeting_min)} candidates meet minimum matched peaks ({min_matched_peaks} of {meas_peaks_count}), using all candidates"
                )
            ...
            candidates_with_peaks = [
                r for r in top_candidates if r.mean_delta_nm < 1000.0
            ]

            # CRITICAL: Filter by oscillation ratio (prefer 0.7 to 1.5 range)
            candidates_good_amplitude = [
                r for r in candidates_with_peaks if 0.7 <= r.oscillation_ratio <= 1.5
            ]
            ...
            candidates_valid_peak_count = [
                r
                for r in candidates_good_amplitude
                if r.theoretical_peaks <= max_theo_peaks
            ]
            ...
            if not candidates_valid_peak_count and candidates_good_amplitude:
                ...
                logger.warning(
                    f"⚠️ No candidates with theo_peaks <= {max_theo_peaks}, relaxing constraint"
                )
            ...
            if candidates_valid_peak_count:
                ...
                candidates_meeting_all = [
                    r
                    for r in candidates_valid_peak_count
                    if r.matched_peaks >= min_matched_peaks
                ]
                if candidates_meeting_all:
                    ...
                    top_matched = [
                        c
                        for c in candidates_meeting_all
                        if c.matched_peaks >= max_matched - 1
                    ]
                    ...
                    max_matched = max(
                        c.matched_peaks for c in candidates_valid_peak_count
                    )
                    top_matched = [
                        c
                        for c in candidates_valid_peak_count
                        if c.matched_peaks >= max_matched - 1
                    ]
                    best_candidate = min(top_matched, key=lambda x: x.mean_delta_nm)
                logger.info(
                    f"✅ Found {len(candidates_valid_peak_count)} candidates with good amplitude AND valid peak count (theo <= {max_theo_peaks})"
                )
            elif candidates_with_peaks:
                # Fallback: has peaks but other filters not met
                # Still try to filter by peak count
                candidates_valid_in_fallback = [
                    r
                    for r in candidates_with_peaks
                    if r.theoretical_peaks <= max_theo_peaks
                ]
                if candidates_valid_in_fallback:
                    best_candidate = max(
                        candidates_valid_in_fallback,
                        key=lambda x: (x.matched_peaks, -x.mean_delta_nm),
                    )
                else:
                    best_candidate = max(
                        candidates_with_peaks,
                ...
                logger.warning(
                    f"⚠️ No candidates with good amplitude, using best peak match (osc_ratio={best_candidate.oscillation_ratio:.2f}, theo_peaks={best_candidate.theoretical_peaks})"
                )
            else:
                # No candidates with peak data
                results = candidate_pool[:top_k]
                logger.info(
                    f"✅ Fine search completed. Best refined score: {results[0].score:.4f} (no peak alignment data available)"
                )
            ...
            if candidates_with_peaks or candidates_good_amplitude:
                ...
                results = [best_candidate] + [
                    r for r in candidate_pool if r != best_candidate
                ]
                results = results[:top_k]
                logger.info(
                    f"✅ Fine search completed. Selected best candidate: Score={best_candidate.score:.4f}, Peak Count Delta={best_candidate.peak_count_delta}, Matched Peaks={best_candidate.matched_peaks}, Mean Delta={best_candidate.mean_delta_nm:.2f}nm, Osc Ratio={best_candidate.oscillation_ratio:.2f} (from {len(candidates_with_peaks)} candidates)"
                )
    ...
    def _run_full_grid_search(
        ...
        logger.info(
            f"   User input step sizes: Lipid={lipid_range[2]:.1f}nm, Aqueous={aqueous_range[2]:.1f}nm, Roughness={roughness_range[2]:.1f}Å"
        )
        ...
        logger.info(
            f"   Calculated fine steps: Lipid={fine_lipid_step:.1f}nm (20% of {lipid_range[2]:.1f}nm), Aqueous={fine_aqueous_step:.1f}nm (20% of {aqueous_range[2]:.1f}nm), Roughness={fine_roughness_step:.1f}Å (20% of {roughness_range[2]:.1f}Å)"
        )
        logger.info(
            f"🔍 Running FULL GRID SEARCH with fine steps: Lipid={fine_lipid_step:.1f}nm, Aqueous={fine_aqueous_step:.1f}nm, Roughness={fine_roughness_step:.1f}Å"
        )
        ...
        aqueous_values = np.arange(
            aqueous_range[0], aqueous_range[1] + 1, fine_aqueous_step
        )
        roughness_values_angstrom = np.arange(
            roughness_range[0], roughness_range[1] + 1, fine_roughness_step
        )
        ...
        total = len(lipid_values) * len(aqueous_values) * len(roughness_values_angstrom)
        logger.info(
            f"📊 Full grid search: {total:,} total combinations to evaluate (no limit - exhaustive search)"
        )
        logger.info(
            f"   Final grid: {len(lipid_values)} lipid × {len(aqueous_values)} aqueous × {len(roughness_values_angstrom)} roughness"
        )
    ...
    def _run_dynamic_search(
        ...
        logger.info("")
        logger.info(
            "🔧 Generating coarse parameter grid for Stage 1 with user step sizes..."
        )
        lipid_values_coarse = np.arange(
            lipid_range[0], lipid_range[1] + 1, coarse_lipid_step
        )
        aqueous_values_coarse = np.arange(
            aqueous_range[0], aqueous_range[1] + 1, coarse_aqueous_step
        )
        roughness_values_coarse = np.arange(
            roughness_range[0], roughness_range[1] + 1, coarse_roughness_step
        )

        logger.info(
            f"   Generated before filtering: Lipid={len(lipid_values_coarse)}, Aqueous={len(aqueous_values_coarse)}, Roughness={len(roughness_values_coarse)}"
        )

        # Filter to user-specified ranges (already bounded by ADOM limits in UI)
        lipid_values_coarse = lipid_values_coarse[
            (lipid_values_coarse >= lipid_range[0])
            & (lipid_values_coarse <= lipid_range[1])
        ]
        aqueous_values_coarse = aqueous_values_coarse[
            (aqueous_values_coarse >= aqueous_range[0])
            & (aqueous_values_coarse <= aqueous_range[1])
        ]
        roughness_values_coarse = roughness_values_coarse[
            (roughness_values_coarse >= roughness_range[0])
            & (roughness_values_coarse <= roughness_range[1])
        ]

        logger.info(
            f"   After range filtering: Lipid={len(lipid_values_coarse)}, Aqueous={len(aqueous_values_coarse)}, Roughness={len(roughness_values_coarse)}"
        )

        coarse_total = (
            len(lipid_values_coarse)
            * len(aqueous_values_coarse)
            * len(roughness_values_coarse)
        )
        logger.info(f"📈 Coarse Grid Dimensions:")
        logger.info(
            f"   - Lipid values: {len(lipid_values_coarse)} ({lipid_values_coarse[0]:.1f} to {lipid_values_coarse[-1]:.1f} nm)"
        )
        logger.info(
            f"   - Aqueous values: {len(aqueous_values_coarse)} ({aqueous_values_coarse[0]:.1f} to {aqueous_values_coarse[-1]:.1f} nm)"
        )
        logger.info(
            f"   - Roughness values: {len(roughness_values_coarse)} ({roughness_values_coarse[0]:.1f} to {roughness_values_coarse[-1]:.1f} Å)"
        )
        ...
        if stage1_budget is not None and coarse_total > stage1_budget:
            logger.warning(
                f"⚠️ Coarse search ({coarse_total:,} combinations) exceeds Stage 1 budget ({stage1_budget:,})"
            )
            logger.warning(
                f"   Limiting coarse search to {stage1_budget:,} combinations (70% of {max_combinations:,} total)..."
            )
            logger.info(
                f"   Before sampling: {len(lipid_values_coarse)} lipid × {len(aqueous_values_coarse)} aqueous × {len(roughness_values_coarse)} roughness = {coarse_total:,} combinations"
            )
            ...
            if current_estimate < stage1_budget:
                ...
                if dim3_count > dim3_target:
                    dim3_target = min(
                        dim3_count,
                        int(np.ceil(stage1_budget / (smallest_count * dim2_target))),
                    )
                elif dim2_count > dim2_target:
                    dim2_target = min(
                        dim2_count,
                        int(np.ceil(stage1_budget / (smallest_count * dim3_target))),
                    )
            ...
            if smallest_name == "lipid":
                ...
                if dim2_name == "aqueous":
                    if dim2_target < aqueous_count:
                        indices = np.linspace(
                            0, aqueous_count - 1, dim2_target, dtype=int
                        )
                        aqueous_values_coarse = aqueous_values_coarse[indices]
                        logger.info(
                            f"   Aqueous sampled: {aqueous_before} → {len(aqueous_values_coarse)} values"
                        )
                    if dim3_target < roughness_count:
                        indices = np.linspace(
                            0, roughness_count - 1, dim3_target, dtype=int
                        )
                        roughness_values_coarse = roughness_values_coarse[indices]
                        logger.info(
                            f"   Roughness sampled: {roughness_before} → {len(roughness_values_coarse)} values"
                        )
                else:  # dim2 is roughness
                    if dim2_target < roughness_count:
                        indices = np.linspace(
                            0, roughness_count - 1, dim2_target, dtype=int
                        )
                        roughness_values_coarse = roughness_values_coarse[indices]
                        logger.info(
                            f"   Roughness sampled: {roughness_before} → {len(roughness_values_coarse)} values"
                    ...
                    if dim3_target < aqueous_count:
                        indices = np.linspace(
                            0, aqueous_count - 1, dim3_target, dtype=int
                        )
                        aqueous_values_coarse = aqueous_values_coarse[indices]
                        logger.info(
                            f"   Aqueous sampled: {aqueous_before} → {len(aqueous_values_coarse)} values"
                ...
                if dim2_name == "lipid":
                    if dim2_target < lipid_count:
                        indices = np.linspace(
                            0, lipid_count - 1, dim2_target, dtype=int
                        )
                        lipid_values_coarse = lipid_values_coarse[indices]
                        logger.info(
                            f"   Lipid sampled: {lipid_before} → {len(lipid_values_coarse)} values"
                        )
                    if dim3_target < roughness_count:
                        indices = np.linspace(
                            0, roughness_count - 1, dim3_target, dtype=int
                    ...
                    if dim3_target < lipid_count:
                        indices = np.linspace(
                            0, lipid_count - 1, dim3_target, dtype=int
                        )
                        lipid_values_coarse = lipid_values_coarse[indices]
                        logger.info(
                            f"   Lipid sampled: {lipid_before} → {len(lipid_values_coarse)} values"
            ...
            logger.info(
                f"   After sampling: {len(lipid_values_coarse)} lipid × {len(aqueous_values_coarse)} aqueous × {len(roughness_values_coarse)} roughness = {coarse_total:,} combinations (target: {stage1_budget:,})"
            )

        logger.info(
            f"🚀 Starting parallel evaluation with {os.cpu_count() or 4} workers..."
        )
        ...
        coarse_results = self._evaluate_parameter_grid(
            ...
            measured,
            lipid_values_coarse,
            aqueous_values_coarse,
            roughness_values_coarse,
        ...
        for i, result in enumerate(coarse_results[:5], 1):
            logger.info(
                f"   {i}. Score: {result.score:.4f}, Corr: {result.correlation:.3f}, RMSE: {result.rmse:.5f}, "
        ...
        logger.info(
            f"🎯 Refining around {len(top_candidates)} top candidates with adaptive step sizes..."
        )
        ...
        for rank, candidate in enumerate(top_candidates):
            ...
            logger.info(
                f"   Candidate #{rank + 1} (Score: {candidate.score:.4f}) - Strategy: {strategy_name}"
            )
            logger.info(
                f"      Center: L={candidate.lipid_nm:.1f}nm, A={candidate.aqueous_nm:.1f}nm, R={candidate.mucus_nm:.1f}Å"
            )
            logger.info(
                f"      Window: ±{lipid_window:.1f}nm (L), ±{aqueous_window:.1f}nm (A), ±{roughness_window:.1f}Å (R)"
            )
            logger.info(
                f"      Step sizes: {fine_lipid_step:.1f}nm (L), {fine_aqueous_step:.1f}nm (A), {fine_roughness_step:.1f}Å (R)"
            )
            ...
            roughness_min = max(
                roughness_range[0], candidate.mucus_nm - roughness_window
            )
            roughness_max = min(
                roughness_range[1], candidate.mucus_nm + roughness_window
            )

            # Generate grid points
            fine_lipids = np.arange(
                lipid_min, lipid_max + fine_lipid_step, fine_lipid_step
            )
            fine_aqueous = np.arange(
                aqueous_min, aqueous_max + fine_aqueous_step, fine_aqueous_step
            )
            fine_roughness = np.arange(
                roughness_min, roughness_max + fine_roughness_step, fine_roughness_step
            )

            # Filter to user-specified ranges (already bounded by ADOM limits in UI)
            fine_lipids = fine_lipids[
                (fine_lipids >= lipid_range[0]) & (fine_lipids <= lipid_range[1])
            ]
            fine_aqueous = fine_aqueous[
                (fine_aqueous >= aqueous_range[0]) & (fine_aqueous <= aqueous_range[1])
            ]
            fine_roughness = fine_roughness[
                (fine_roughness >= roughness_range[0])
                & (fine_roughness <= roughness_range[1])
            ]
            ...
            for lipid in fine_lipids:
                for aqueous in fine_aqueous:
                    for roughness_angstrom in fine_roughness:
                        ...
                        if (
                            abs(lipid - candidate.lipid_nm) < fine_lipid_step * 0.5
                            and abs(aqueous - candidate.aqueous_nm)
                            < fine_aqueous_step * 0.5
                            and abs(roughness_angstrom - candidate.mucus_nm)
                            < fine_roughness_step * 0.5
                        ):
                            continue
            ...
            logger.info(
                f"      Generated {candidate_combinations:,} combinations around this candidate"
            )
        ...
        logger.info(
            f"   - Stage 1 budget: {stage1_budget:,} combinations (70% of total)"
        )
        logger.info(f"   - Stage 1 used: {stage1_used:,} combinations")
        logger.info(f"   - Stage 1 unused: {unused_stage1_budget:,} combinations")
        logger.info(
            f"   - Stage 2 base budget: {stage2_budget:,} combinations (30% of total)"
        )
        logger.info(
            f"   - Stage 2 total budget: {remaining_budget:,} combinations (includes unused Stage 1 budget)"
        )

        if max_combinations and total_dynamic > remaining_budget:
            logger.warning(
                f"⚠️ Stage 2 combinations ({total_dynamic:,}) exceed remaining budget ({remaining_budget:,})"
            )
            logger.warning(
                f"   Limiting to {remaining_budget:,} combinations by random sampling..."
            )
            ...
            logger.warning(
                f"⚠️ Total combinations ({total_dynamic:,}) exceed max limit ({max_combinations:,})"
            )
            logger.warning(
                f"   Limiting to {max_combinations:,} combinations by random sampling..."
            )
        ...
        logger.info(
            f"🚀 Starting parallel evaluation of {total_dynamic:,} combinations with {os.cpu_count() or 4} workers..."
        )
        ...
        with ProcessPoolExecutor(
            ...
            for future in as_completed(futures):
                completed_count += 1
                if (
                    completed_count % max(1, total_dynamic // 10) == 0
                    or completed_count == total_dynamic
                ):
                    ...
                    logger.info(
                        f"   Progress: {completed_count}/{total_dynamic} ({progress_pct:.1f}%) | "
                        f"Elapsed: {elapsed:.1f}s | Rate: {rate:.1f} comb/s | ETA: {eta:.1f}s"
                    )
        ...
        if candidates_valid_peak_count:
            ...
            # Reorder results to put best candidate first
            final_results = [best_candidate] + [
                r for r in candidate_pool if r != best_candidate
            ]
            final_results = final_results[:top_k]
            logger.info(
                f"✅ Selected best candidate: Score={best_candidate.score:.4f}, Theo Peaks={best_candidate.theoretical_peaks}, Matched Peaks={best_candidate.matched_peaks}, Mean Delta={best_candidate.mean_delta_nm:.2f}nm, Osc Ratio={best_candidate.oscillation_ratio:.2f}"
            )
        elif candidates_with_peaks:
            # Fallback: has peaks but other filters not met
            candidates_valid_in_fallback = [
            ...
            logger.warning(
                f"⚠️ No candidates with good amplitude, using best peak match (theo_peaks={best_candidate.theoretical_peaks}, osc_ratio={best_candidate.oscillation_ratio:.2f})"
            )
            # Reorder results to put best candidate first
            final_results = [best_candidate] + [
                r for r in candidate_pool if r != best_candidate
            ...
            final_results = final_results[:top_k]
            logger.info(
                f"✅ Selected best candidate: Score={best_candidate.score:.4f}, Theo Peaks={best_candidate.theoretical_peaks}, Matched Peaks={best_candidate.matched_peaks}, Mean Delta={best_candidate.mean_delta_nm:.2f}nm"
            )
        else:
        ...
        if final_results:
            logger.info(
                f"🏆 Best result: Score={final_results[0].score:.4f}, Corr={final_results[0].correlation:.3f}, "
        ...
        if final_results and final_results[0].mean_delta_nm > 2.5:
            ...
            ultra_fine_total = (
                len(ultra_fine_lipid)
                * len(ultra_fine_aqueous)
                * len(ultra_fine_roughness)
            )
            logger.info(f"📊 Ultra-fine grid: {ultra_fine_total:,} combinations")
            logger.info(
                f"   - Lipid: {len(ultra_fine_lipid)} values ({ultra_fine_lipid[0]:.1f} to {ultra_fine_lipid[-1]:.1f} nm, step 2nm)"
            )
            logger.info(
                f"   - Aqueous: {len(ultra_fine_aqueous)} values ({ultra_fine_aqueous[0]:.1f} to {ultra_fine_aqueous[-1]:.1f} nm, step 20nm)"
            )
            logger.info(
                f"   - Roughness: {len(ultra_fine_roughness)} values ({ultra_fine_roughness[0]:.1f} to {ultra_fine_roughness[-1]:.1f} Å, step 50Å)"
            )
            ...
            if ultra_fine_results:
                ...
                if valid_results:
                    ...
                    top_matched = [
                        r for r in valid_results if r.matched_peaks >= max_matched - 1
                    ]
                    best_ultra = min(top_matched, key=lambda x: x.mean_delta_nm)

                    if best_ultra.mean_delta_nm < best.mean_delta_nm:
                        logger.info(
                            f"✅ Ultra-fine improved: Mean Delta {best.mean_delta_nm:.2f}nm → {best_ultra.mean_delta_nm:.2f}nm"
                        )
                        final_results = [best_ultra] + [
                            r for r in valid_results if r != best_ultra
                        ][: top_k - 1]
                    else:
                        logger.info(
                            f"ℹ️ Ultra-fine did not improve (current: {best.mean_delta_nm:.2f}nm, best found: {best_ultra.mean_delta_nm:.2f}nm)"
                        )
    ...
    def _evaluate_parameter_grid(
        ...
        if filtered_out > 0:
            logger.info(
                f"📊 Filtered out {filtered_out} results (correlation < {min_correlation_filter} or RMSE > 0.002)"
            )
        ...
        # Filter candidates that meet minimum matched peaks requirement
        candidates_meeting_min = [
            r for r in results if r.matched_peaks >= min_matched_peaks
        ]
        ...
        if candidates_valid_peak_count:
            ...
            logger.warning(
                f"⚠️ No candidates with good amplitude, using best peak match (theo_peaks={best_candidate.theoretical_peaks})"
            )
            # Reorder results to put best candidate first
            final_results = [best_candidate] + [
                r for r in candidate_pool if r != best_candidate
...
def demo():
    ...
    for i, r in enumerate(coarse_results[:5], 1):
        print(
            f"{i:<6} {r.lipid_nm:<12.1f} {r.aqueous_nm:<14.1f} {r.mucus_nm:<14.1f} {r.score:<10.4f} {r.rmse:<10.6f} {r.crossing_count:<10}"
        )
    ...
    if coarse_results:
        ...
        plt.subplot(1, 2, 2)
        residual = (
            measured[: len(best.theoretical_spectrum)] - best.theoretical_spectrum
        )
        ...
        plt.ylabel("Residual")
        plt.title(
            f"Residual (Crossings: {best.crossing_count}, Score: {best.score:.3f})"
        )
        plt.grid(True, alpha=0.3)
