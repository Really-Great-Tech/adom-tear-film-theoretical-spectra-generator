...
if selected_file and Path(selected_file).exists():
    try:
        ...
        shlomo_params = (
            get_shlomo_params(Path(selected_file).name) if selected_file else None
        )
        ...
        if shlomo_params:
            ...
            try:
                theo_shlomo = grid_search.calculate_theoretical_spectrum(
                    wavelengths,
                    shlomo_params["lipid_shlomo_nm"],
                    shlomo_params["aqueous_shlomo_nm"],
                    roughness_nm,
                )
                if focus_mask.sum() > 0 and np.std(theo_shlomo[focus_mask]) > 1e-10:
                    scale_s = np.dot(
                        measured[focus_mask], theo_shlomo[focus_mask]
                    ) / np.dot(theo_shlomo[focus_mask], theo_shlomo[focus_mask])
                    theo_shlomo_aligned = theo_shlomo * scale_s
                else:
                    theo_shlomo_aligned = theo_shlomo
                theo_shlomo_display = theo_shlomo_aligned[wl_mask]
                score_shlomo = calculate_peak_based_score(
                    wl_display, meas_display, theo_shlomo_display
                )
                deviation_at_shlomo = {
        ...
        if sp:
            with st.expander("📋 Shlomo comparison (ground truth)", expanded=True):
                ...
                with col2:
                    ...
                    st.metric(
                        "Our aqueous (nm)", f"{computed_data['display_aqueous']:.0f}"
                    )
                with col3:
                ...
                with col_a:
                    ...
                    if loss_ours:
                        st.metric(
                            "Score (higher = better)",
                            f"{loss_ours.get('score', 0):.3f}",
                        )
                        st.metric(
                            "RMSE (lower = better)", f"{loss_ours.get('rmse', 0):.6f}"
                        )
                        st.metric(
                            "Correlation", f"{loss_ours.get('correlation', 0):.3f}"
                        )
                with col_b:
                    ...
                    if dev_shlomo and isinstance(dev_shlomo, dict):
                        st.metric(
                            "Score (higher = better)",
                            f"{dev_shlomo.get('score', 0):.3f}",
                        )
                        st.metric(
                            "RMSE (lower = better)", f"{dev_shlomo.get('rmse', 0):.6f}"
                        )
                        st.metric(
                            "Correlation", f"{dev_shlomo.get('correlation', 0):.3f}"
                        )
                st.markdown("---")
                ...
                if loss_ours and dev_shlomo and isinstance(dev_shlomo, dict):
                    ...
                    if ours_better_rmse and ours_better_corr:
                        ...
                        st.warning(
                            "By **RMSE and correlation**, the waveform fit is **better** at Shlomo's values than at our optimum — our fit is worse here. Our grid search may have landed in a different basin, or his method captures something our metric doesn't. Worth checking the spectrum and amplitude plots."
                        )
                    elif (
                        abs(rmse_ours - rmse_shlomo) < 1e-6
                        and abs(corr_ours - corr_shlomo) < 0.02
                    ):
                        st.success(
                            "Both points have **similar** waveform fit (RMSE and correlation). We may be in the same solution basin; the parameter difference could be within acceptable variation or different but valid optima."
                        )
                    else:
                        st.caption(
                            "**Mixed:** RMSE and correlation disagree (e.g. we have lower RMSE but lower correlation, or vice versa). For direct waveform match, prefer **lower RMSE** and **higher correlation**. Composite score can differ because it also weights peak count and alignment."
                        )
                else:
                    st.caption(
                        "Compare RMSE (lower = better) and correlation (higher = better) above. Those are the direct waveform-fit metrics; composite score can sometimes disagree when it weights other factors."
                    )
                st.markdown("---")
                st.markdown("**Loss landscape**")
                st.caption(
                    "1D: loss vs aqueous (lipid fixed at Shlomo's). 2D: aqueous vs lipid, color = RMSE (loss). Identifies local minima and whether Shlomo's point lies in a low-loss region."
                )
                if st.button(
                    "Generate 1D & 2D loss landscape", key="btn_loss_landscape"
                ):
                    with st.spinner("Computing loss over grid (1D + 2D)..."):
                        ...
                        def loss_at(lipid_nm, aqueous_nm):
                            try:
                                theo = grid_search.calculate_theoretical_spectrum(
                                    wavelengths, lipid_nm, aqueous_nm, roughness_nm
                                )
                                if (
                                    focus_mask.sum() > 0
                                    and np.std(theo[focus_mask]) > 1e-10
                                ):
                                    scale = np.dot(
                                        measured[focus_mask], theo[focus_mask]
                                    ) / np.dot(theo[focus_mask], theo[focus_mask])
                                    theo = theo * scale
                                theo_d = theo[wl_mask]
                                res = calculate_peak_based_score(
                                    wl_display, meas_display, theo_d
                                )
                ...
                if cache_key in st.session_state:
                    ...
                    fig_1d.add_vline(
                        ...
                        line_dash="dash",
                        line_color="#db2777",
                        annotation_text="Shlomo",
                    )
                    fig_1d.add_vline(
                        x=A_o,
                        line_dash="dot",
                        line_color="#2563eb",
                        annotation_text="Ours",
                    )
                    fig_1d.update_layout(
                        title="1D: Loss vs aqueous thickness (lipid fixed at Shlomo's)",
                        xaxis_title="Aqueous (nm)",
                    ...
                    fig_2d.add_trace(
                        go.Scatter(
                            ...
                            mode="markers+text",
                            name="Shlomo",
                            marker=dict(symbol="x", size=14, color="#db2777"),
                            text=["Shlomo"],
                    ...
                    fig_2d.add_trace(
                        go.Scatter(
                            ...
                            mode="markers+text",
                            name="Ours",
                            marker=dict(symbol="circle", size=12, color="#2563eb"),
                            text=["Ours"],
                            textposition="top center",
                        )
                    )
